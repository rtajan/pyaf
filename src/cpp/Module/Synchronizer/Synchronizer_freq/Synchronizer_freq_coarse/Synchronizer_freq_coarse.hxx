...
namespace module
{
	...
	: Module(), N(N), is_active(false), curr_idx(0)
	{
		...
		auto &p2 = this->create_task("set_del");
		auto p2s_DEL  = this->template create_socket_in<int32_t>(p2, "DEL" , 1      );
		this->create_codelet(p2, [p2s_DEL](Module &m, Task &t, const size_t frame_id) -> int
		{
			auto& coarse_sync = static_cast<Synchronizer_freq_coarse<R>&>(m);
			int N = coarse_sync.get_N();
			int* DEL = static_cast<int*>(t[p2s_DEL].get_dataptr());
			coarse_sync.set_curr_idx((N - *DEL) % (N / 2));
			return 0;
		});
	}
